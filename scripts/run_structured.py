#!/usr/bin/env python3
"""Phase 8 structured-output reliability benchmark.

Usage:  python scripts/run_structured.py --models all
"""
...
def main() -> int:
    ...
    runner = StructuredRunner(client, sample_metrics=not args.no_metrics)
    print(f"== structured output: {len(models)} model(s), run_id={runner.run_id} ==")
    for m in models:
        print(f"\n# {m.display_name}")
        try:
            runner.run_model(m)
